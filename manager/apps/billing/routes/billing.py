...
from manager.libs.utils.util_json import render_json
from manager.libs.decorators import subscription_required, handle_stripe_exceptions
from manager.apps.billing.forms import CreditCardForm, UpdateSubscriptionForm, CancelSubscriptionForm
from manager.models.Billing.coupon import Coupon

billing = Blueprint('billing', __name__, url_prefix='/subscription')
...
def pricing():
    ...
    return render_template('apps/billing/pricing.html', form=form, plans=settings.DevConfig.STRIPE_PLANS)
...
def create():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('users.settings'))
    return render_template('apps/billing/payment_method.html', form=form, plan=subscription_plan)
...
def update_payment_method():
    ...
    if form.validate_on_submit():
        ...
        updated = subscription.\
                        update_payment_method(user=current_user,
                                              credit_card=card,
                                              name=request.form.get('name'),
                                              token=request.form.get('stripe_token'))
        if updated:
            ...
            flash('You must enable JavaScript for this request.', 'warning')
        return redirect(url_for('user.settings'))
    return render_template('billing/payment_method.html', form=form, plan=active_plan,
...
def billing_details():
    ...
    return render_template('billing/billing_details.html', invoices=invoices, upcoming=upcoming, coupon=coupon)
